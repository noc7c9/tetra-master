...
fn expectiminimax_search(state: State) -> Action {
    ...
    let (_value, action) = max_value(state);
    action.unwrap()
}

fn max_value(state: State) -> (f32, Option<Action>) {
    if state.logging_enabled {
        println!("{}MAX NODE", state.indent);
    }
    if state.is_terminal() {
        let terminal_value = state.utility();
        if state.logging_enabled {
            let label = match terminal_value.total_cmp(&0.0) {
    ...
    let mut curr_value = f32::MIN;
    let mut selected_action = None;
    for action in state.get_actions() {
        let new_state = state.apply_action(action);

        if state.logging_enabled {
            println!("{}action: {action:?}", new_state.indent);
        }

        let new_state_value = if let Status::WaitingResolveBattle(_) = new_state.status {
            chance_value(new_state).0
        } else if new_state.turn == state.player {
            max_value(new_state).0
        } else {
            min_value(new_state).0
        };
    ...
    if state.logging_enabled {
        println!(
            "{}MAX NODE value = {curr_value} | selected_action = {selected_action:?}",
            state.indent
        );
    }
...
fn min_value(state: State) -> (f32, Option<Action>) {
    if state.logging_enabled {
        println!("{}MIN NODE", state.indent);
    }
    if state.is_terminal() {
        let terminal_value = state.utility();
        if state.logging_enabled {
    ...
    if state.logging_enabled {
        println!(
            "{}MIN NODE value = {curr_value} | selected_action = {selected_action:?}",
            state.indent
        );
    }
    (curr_value, selected_action)
...
fn chance_value(state: State) -> (f32, Option<Action>) {
    if state.logging_enabled {
        println!("{}CHANCE NODE", state.indent);
    }
    let mut sum_value = 0.0;
    for resolution in state.get_resolutions() {
        let new_state = state.apply_resolution(resolution);

        if state.logging_enabled {
            println!("{}resolution: {resolution:?}", new_state.indent);
        }
    ...
    if state.logging_enabled {
        println!("{}CHANCE NODE value = {sum_value}", state.indent);
    }
    (sum_value, None)
}
