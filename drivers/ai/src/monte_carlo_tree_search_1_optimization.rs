...
impl super::Ai for Ai {
    ...
    fn apply_place_card(&mut self, cmd: core::PlaceCard) {
        ...
        let id = NodeId::PlaceCard(card, cell);
...
impl PartialEq for NodeId {
    fn eq(&self, other: &Self) -> bool {
        match (&self, &other) {
            ...
            (Self::PlaceCard(a1, a2), Self::PlaceCard(b1, b2)) => a1 == b1 && a2 == b2,
            (Self::PickBattle(a), Self::PickBattle(b)) => a == b,
...
impl Node {
    ...
    fn expand(&mut self, prealloc: &mut PreAlloc, con: &ConstantState) {
        ...
        match &self.var.status {
            ...
            Status::WaitingPlaceCard => {
                let iter = self.var.get_place_card_actions(con, prealloc);
                self.children.reserve_exact(iter.len());
                for (card, cell) in iter {
                    let new_var = self.var.apply_place_card(con, card, cell);
                    let child = Node::new(owner, NodeId::PlaceCard(card, cell), new_var);
                    self.children.push(child);
                }
            }
            Status::WaitingPickBattle { .. } => {
                let iter = self.var.get_pick_battle_actions();
                self.children.reserve_exact(iter.len());
                for cell in iter {
                    let new_var = self.var.apply_pick_battle(cell);
                    let child = Node::new(owner, NodeId::PickBattle(cell), new_var);
                    self.children.push(child);
                }
            }
            Status::WaitingResolveBattle(_) => {
                let iter = self.var.get_resolutions(con);
                self.children.reserve_exact(iter.len());
    ...
    fn uct_value(&self, c_value: f32, parent_visits: u32) -> f32 {
        if self.visits == 0 {
            f32::INFINITY
        } else {
    ...
    fn print_depth_limited(&self, con: &ConstantState, max_depth: usize) {
        ...
        fn print_node(
            ...
            match node.id {
                ...
                NodeId::PlaceCard(card, cell) => {
                    print!("Node({:?} | {card:?} on {cell:?})", node.owner.unwrap())
                }
                NodeId::PickBattle(cell) => print!("Node({:?} | {cell:?})", node.owner.unwrap()),
...
fn monte_carlo_tree_search(
    ...
    fn simulate_random_playout(
        ...
        loop {
            match &var.status {
                ...
                Status::WaitingPickBattle { .. } => {
                    let cell = var.get_pick_battle_actions().choose(rng).unwrap();
    ...
    match root.get_child_with_most_visits().id {
        NodeId::PlaceCard(card, cell) => crate::Action::PlaceCard(core::PlaceCard {
            card: card.0,
            cell: cell.0,
            player,
        }),
        NodeId::PickBattle(cell) => crate::Action::PickBattle(core::PickBattle {
            cell: cell.0,
            player,
        }),
        _ => unreachable!(),
...
fn prealloc(cmd: &core::Setup) -> PreAlloc {
...
impl VariableState {
    ...
    ) -> impl ExactSizeIterator<Item = PlaceCard> + 'c {
        actions.clear();
    ...
    fn get_pick_battle_actions(&self) -> impl ExactSizeIterator<Item = PickBattle> {
    ...
    fn apply_place_card(&self, con: &ConstantState, card: CardHandIdx, cell: CellIdx) -> Self {
    ...
    fn apply_pick_battle(&self, cell: CellIdx) -> Self {
        let mut clone = self.clone();

        clone.handle_pick_battle(cell);
