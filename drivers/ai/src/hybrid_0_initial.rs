...
impl AI {
    pub fn reinit(&mut self, player: core::Player, setup: &core::Setup) {
        ...
        self.con = ConstantState::new(max_time_ms, c_value, max_depth, prob_cutoff, player, setup);
    ...
    fn has_switched(&self) -> bool {
        self.move_number >= self.switch_point
    }
}
...
impl VariableState {
    ...
    fn handle_action(&mut self, con: &ConstantState, action: Action) {
        match action {
            Action::PlaceCard { cell, card } => {
                self.handle_place_card(con, cell, card);
                self.depth += 1;
            }
