...
mod tests {
           ...
           mod $name {
               ...
               fn sanity_check_7_pick_more_likely_battle_when_using_non_deterministic_battle_systems() {
                   for battle_system in [
                       ...
                       //  As both cards point at each other winning either battle would skip the
                       //  other battle, so by defeating the weaker card, the stronger card won't
                       //  have to be fought
    ...
    test!(expectiminimax_0_naive, 3);
    test!(expectiminimax_1_simplify, 3);
    test!(expectiminimax_2_ab_pruning, 3);
    test!(expectiminimax_3_negamax, 3);
