...
impl State {
    ...
    fn get_resolutions(&self) -> arrayvec::ArrayVec<Resolution, 2> {
        ...
        match self.status {
            Status::WaitingResolveBattle(WaitingResolveBattle { attacker, defender }) => {
                let mut attack_win_prob =
                    lookup(self.battle_system, attacker.value, defender.value);

                if attack_win_prob < self.prob_cutoff {
                    attack_win_prob = 0.0;
                }
                if attack_win_prob > (1.0 - self.prob_cutoff) {
                    attack_win_prob = 1.0;
                }
