...
impl super::Ai for Ai {
    ...
    fn apply_place_card(&mut self, cmd: core::PlaceCard) {
        self.var.handle_place_card(&self.con, cmd);
    }

    fn apply_pick_battle(&mut self, cmd: core::PickBattle) {
        self.var.handle_pick_battle(&self.con, cmd);
    }
...
impl ConstantState {
    ...
    fn get_card(&self, card_idx: CardIdx) -> core::Card {
        self.cards[card_idx.get() as usize]
    }
}
...
impl VariableState {
    fn new(cmd: &core::Setup) -> Self {
        ...
        for cell in cmd.blocked_cells {
            board[cell as usize] = CellPacked::blocked();
        }
...
type Hand = [Option<CardIdx>; core::HAND_SIZE];
type Board = [CellPacked; core::BOARD_SIZE];

// An index into the ConstantState.cards array
// the actual index is stored in the leftmost 4 bits, the rightmost 4 bits are ignored
// It also uses NonZeroU8 so that it can be wrapped with an Option without increasing in size
#[derive(Clone, Copy, PartialEq)]
struct CardIdx(NonZeroU8);

impl std::fmt::Debug for CardIdx {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "CardIdx({:08b})", self.0)
    }
}

impl CardIdx {
    fn new(raw_idx: u8) -> Self {
        let idx = raw_idx << 4;
        debug_assert!(idx >> 4 == raw_idx);
        Self(NonZeroU8::new(idx | 0b1111).unwrap())
    }
...
// A bit packed enum that represents Cell::Empty, Cell::Blocked, or Cell::Card(owner, CardIdx)
// the high 4 bits is the CardIdx
// the low 4 bits are
//     0b0001 => Empty
//     0b0010 => Blocked
//     0b0100 => Card(Blue, _)
//     0b1000 => Card(Red, _)
#[derive(Clone, Copy, PartialEq)]
struct CellPacked(u8);

impl std::fmt::Debug for CellPacked {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "CardIdx({:04b}_{:04b})", self.0 >> 4, self.0 & 0b1111)
    }
}

impl Default for CellPacked {
    fn default() -> Self {
        CellPacked::empty()
    }
}
...
impl CellPacked {
    ...
    fn card(owner: core::Player, card_idx: CardIdx) -> Self {
        let low = match owner {
            core::Player::Blue => Self::BLUE,
            core::Player::Red => Self::RED,
        };
        let high = card_idx.get() << 4;
        Self(high | low)
    }

    fn is_empty(self) -> bool {
        self.0 & 0b1111 == Self::EMPTY
    }
    ...
    fn unpack(self) -> Cell {
        match self.0 & 0b1111 {
            ...
            Self::BLUE => Cell::Card(OwnedCard {
                owner: core::Player::Blue,
                card: CardIdx::new(self.0 >> 4),
            }),
            Self::RED => Cell::Card(OwnedCard {
...
impl VariableState {
    fn is_terminal(&self, con: &ConstantState) -> bool {
        self.depth >= con.max_depth || matches!(self.status, Status::GameOver)
    }

    fn utility(&self) -> f32 {
        let mut count = 0.0;
        for cell in self.board {
            if let Cell::Card(card) = cell.unpack() {
    ...
    fn get_resolutions(&self, con: &ConstantState) -> arrayvec::ArrayVec<Resolution, 2> {
        ...
        match self.status {
            Status::WaitingResolveBattle(WaitingResolveBattle { attacker, defender }) => {
                let mut attack_win_prob = lookup(con.battle_system, attacker.value, defender.value);
    ...
    fn apply_action(&self, con: &ConstantState, action: Action) -> Self {
        ...
        match action {
            Action::PlaceCard(cmd) => {
                clone.handle_place_card(con, cmd);
                clone.depth += 1;
            }
            Action::PickBattle(cmd) => clone.handle_pick_battle(con, cmd),
        }
    ...
    fn handle_place_card(&mut self, con: &ConstantState, cmd: core::PlaceCard) {
        if let Status::WaitingPlaceCard = self.status {
            ...
            self.board[cmd.cell as usize] = CellPacked::card(self.turn, card);

            self.resolve_interactions(con, cmd.cell);
        }
    }
    ...
    fn handle_resolve_battle(&mut self, con: &ConstantState, winner: core::BattleWinner) {
        if let Status::WaitingResolveBattle(ref status) = self.status {
            ...
            let loser = match self.board[loser_cell as usize].unpack() {
                Cell::Card(card) => card,
                _ => unreachable!(),
            };
            for &(comboed_cell, arrow) in get_possible_neighbours(loser_cell) {
                let comboed = match self.board[comboed_cell as usize].unpack() {
                    Cell::Card(card) => card,
                    _ => continue,
                };
    ...
    fn handle_pick_battle(&mut self, con: &ConstantState, cmd: core::PickBattle) {
    ...
    fn resolve_interactions(&mut self, con: &ConstantState, attacker_cell: u8) {
        let attacker = match self.board[attacker_cell as usize].unpack() {
            Cell::Card(card) => card,
            _ => unreachable!(),
        };
        ...
        for &(defender_cell, arrow) in get_possible_neighbours(attacker_cell) {
            let defender = match self.board[defender_cell as usize].unpack() {
                Cell::Card(card) => card,
                _ => continue,
            };
    ...
    fn flip(&mut self, cell: u8) {
        ...
        let card = match self.board[cell].unpack() {
            Cell::Card(card) => card,
            _ => unreachable!(),
        };
        self.board[cell] = CellPacked::card(card.owner.opposite(), card.card);
    }

    fn resolve_battle(&mut self, con: &ConstantState, attacker_cell: u8, defender_cell: u8) {
        let attacker = match self.board[attacker_cell as usize].unpack() {
            Cell::Card(owned) => con.get_card(owned.card),
            _ => unreachable!(),
        };
        let defender = match self.board[defender_cell as usize].unpack() {
            Cell::Card(owned) => con.get_card(owned.card),
